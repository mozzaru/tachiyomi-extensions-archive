...
class MGKomik : Madara("MG Komik", "https://id.mgkomik.cc", "id", SimpleDateFormat("dd MMM yy", Locale.US)) {
    ...
    override val client: OkHttpClient = super.client.newBuilder()
        ...
        .build().also {
            Log.d(TAG, "OkHttpClient initialized with rate limit")
        }

    override fun headersBuilder(): Headers.Builder {
        Log.d(TAG, "Generating headers with random string: $randomString")
    ...
    private fun generateRandomString(length: Int): String {
        ...
        val result = (1..length)
            .map { charset.random() }
            .joinToString("")
        Log.d(TAG, "Generated random string: $result")
        return result
    }
    ...
    private val randomLength = Random.Default.nextInt(13, 21).also {
        Log.d(TAG, "Random length for X-Requested-With: $it")
    }
